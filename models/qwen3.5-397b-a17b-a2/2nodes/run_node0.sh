#!/bin/sh
# Node 0 (leader): dp-rank 0 of the DP=2 x TP=8 mixed engine (dp-local=1), plus
# the API server on :8080. Node 1 joins headless with dp-rank 1. Engine flags
# follow the A2 Multi-Node MP reference (5.2 节). 模型路径改为 /root/model、
# --port 8080（参考用 8000）、去掉 VLLM_USE_MODELSCOPE（模型已挂在 /root/model）。
