#!/bin/sh
# Prefill per-instance template (official GLM-5.2 A3 PD guide), called by
...
# Each invocation is one standalone DP=2/TP=16 instance (all 16 NPUs on a node).
# LOCAL_IP / NIC_NAME are exported by run_prefill_node*.sh after NIC resolution
# (the guide hardcodes them).
#
# Deviations from the guide, both deliberate:
#   - model path /root/model (repo convention, not the modelscope cache path)
#   - served-model-name glm-52 is kept from upstream.
# Speculative method deepseek_mtp is GLM-5.2's own MTP per upstream.
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeConnectorV1", "kv_role": "kv_producer", "kv_port": "30000", "engine_id": "0", "kv_connector_extra_config": {"use_ascend_direct": true, "prefill": {"dp_size": 2, "tp_size": 16}, "decode": {"dp_size": 8, "tp_size": 4}}}'
