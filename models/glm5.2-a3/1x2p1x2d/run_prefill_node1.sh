#!/bin/sh
# Prefill node 1 (AISHIPBOX_NODE_RANK=1): DP worker of the prefill engine
# (DP=2 x TP=16) led by prefill node 0. launch_online_dp.py spawns 1 local
# instance (16 NPUs, TP=16) on API port 9081, dp-rank 1, rendezvousing with the
# prefill master (rank 0 = AISHIPBOX_ADDR_0) on rpc port 16591. Same engine_id /
# kv_port as node 0 -- it is the same prefill engine. Guide command mirrored:
#   python launch_online_dp.py --dp-size 2 --tp-size 16 --dp-size-local 1 \
#       --dp-rank-start 1 --dp-address <prefill master ip> --dp-rpc-port 16591 \
#       --vllm-start-port 9081

set -e
: "${AISHIPBOX_CURRENT_ADDR:?run via run.sh}"
: "${AISHIPBOX_ADDR_0:?run via run.sh}"

local_ip="$AISHIPBOX_CURRENT_ADDR"
prefill_master_ip="$AISHIPBOX_ADDR_0"

nic_name=$(ifconfig 2>/dev/null | awk -v ip="$local_ip" '
    /^[^[:space:]]/ { iface=$1; sub(":","",iface) }
    $1=="inet" && $2==ip { print iface; exit }
')
if [ -z "$nic_name" ]; then
    echo "[run] could not find NIC for $local_ip" >&2
    ifconfig >&2 || true
    exit 1
fi
echo "[run] role=PREFILL_1 (external DP=2 x TP=16, 1 local instance) nic=$nic_name local=$local_ip master=$prefill_master_ip"

export LOCAL_IP="$local_ip"
export NIC_NAME="$nic_name"

here=$(cd "$(dirname "$0")" && pwd)

mkdir -p /root/kernel_cache
cd /root/kernel_cache

exec python3 "$here/launch_online_dp.py" \
    --template "$here/run_dp_template_prefill.sh" \
    --dp-size 2 \
    --tp-size 16 \
    --dp-size-local 1 \
    --dp-rank-start 1 \
    --dp-address "$prefill_master_ip" \
    --dp-rpc-port 16591 \
    --vllm-start-port 9081
