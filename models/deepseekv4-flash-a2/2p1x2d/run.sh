#!/bin/sh
# DeepSeek-V4 P-D 分离在 A2（8 NPU/节点）上的入口脚本——2p1x2d：
# 2 个单节点 prefill 引擎 + 1 个横跨 2 节点的 decode 引擎，共 4 个节点。
# source setup_rank_env.sh，并按 rank 分发到对应角色的启动脚本。
# 从一份 4 机 A2 生产部署手册移植而来；详见 meta.yaml。
#
# 拓扑（按 rank table 顺序）：
#   rank 0 -> prefill 0   (独立 DP=8 引擎, kv_producer, kv_port=30000)
#   rank 1 -> prefill 1   (独立 DP=8 引擎, kv_producer, kv_port=30100)
#   rank 2 -> decode  0   (DP=16 master, dp-rank 0..7,   kv_consumer, kv_port=30200)
#   rank 3 -> decode  1   (DP=16 worker, dp-rank 8..15,  kv_consumer, kv_port=30200)
#
# 每个 prefill 节点在自己的 8 张 NPU 上跑 8 个本地 DP worker（TP=1, dp-size-local=8）。
# decode 集群横跨 2 节点，每节点 8 个本地 DP worker（TP=1）。
#
