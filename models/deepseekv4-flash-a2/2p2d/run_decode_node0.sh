#!/bin/sh
# Decode node 0 (AISHIPBOX_NODE_RANK=2): standalone DP=8 engine, kv_role=consumer.
# 8 local DP workers (one per NPU, TP=1), kv_port=30200, engine_id=2.
#
# Unlike the original A3 layout (where decode was a single DP=32 fleet spanning
# 2 nodes via --headless/--data-parallel-start-rank), this engine is its own DP
# leader and never talks to the other decode node — fully symmetric with prefill.
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeHybridConnector", "kv_role": "kv_consumer", "kv_port": "30200", "engine_id": "2", "kv_connector_extra_config": {"prefill": {"dp_size": 8, "tp_size": 1}, "decode": {"dp_size": 8, "tp_size": 1}}}'
