#!/bin/sh
# Prefill per-instance template (official GLM-5.2 A2 PD guide), called by
...
# Each invocation is one standalone DP=4/TP=8 instance (all 8 NPUs on an A2 node).
...
# Guard against the prefill/decode template mixup: prefill is the only TP=8 role.
if [ "$7" != "8" ]; then
    echo "[template] run_dp_template_prefill.sh expects tp-size 8 but got $7 -- wrong template for this role (pass the right one via --template)" >&2
    exit 1
fi
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeConnector", "kv_role": "kv_producer", "kv_port": "30000", "engine_id": "0", "kv_connector_module_path": "vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_connector", "kv_connector_extra_config": {"use_ascend_direct": true, "prefill": {"dp_size": 4, "tp_size": 8}, "decode": {"dp_size": 8, "tp_size": 4}}}' \
