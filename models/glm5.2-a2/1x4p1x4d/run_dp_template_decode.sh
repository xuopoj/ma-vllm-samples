#!/bin/sh
# Decode per-instance template (official GLM-5.2 A2 PD guide), called by
...
# Each invocation is one standalone DP=8/TP=4 instance (4 NPUs). 2 instances per
# decode node; the decode engine spans 4 nodes (DP=8 total).
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeConnector", "kv_role": "kv_consumer", "kv_port": "30100", "engine_id": "1", "kv_connector_module_path": "vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_connector", "kv_connector_extra_config": {"use_ascend_direct": true, "prefill": {"dp_size": 4, "tp_size": 8}, "decode": {"dp_size": 8, "tp_size": 4}}}' \
