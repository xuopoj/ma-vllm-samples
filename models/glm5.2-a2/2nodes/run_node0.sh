#!/bin/sh
# Node 0 (leader): dp-rank 0 of the DP=2 x TP=8 mixed engine, plus the API
# server on :8080. Node 1 joins headless as dp-rank 1. TP=8 spans all 8 NPUs on
# this A2 node; the two nodes form one DP=2 engine.
#
# Derived from the GLM-5.2 tutorial (Multi-Node A2, Node 0). Deviations:
# /root/model and --port 8080 (repo conventions; upstream cache path + 7000).
# Upstream wrote --max_model_len (underscore); normalized to --max-model-len.
