#!/bin/sh
# Decode per-instance template (official A3 1P1D guide), called by
...
# Each invocation is one standalone DP=16/TP=1 instance (1 NPU).
# Budget arithmetic: max-num-batched-tokens 120 = max-num-seqs 60 x 2 tokens
# (1 sampled + 1 MTP speculative).
# LOCAL_IP / NIC_NAME are exported by run_decode_node0.sh after NIC
# resolution (the guide hardcodes them).
#
# Deviations from the guide, both deliberate:
#   - kv_port 36100 (guide: 30100): the official kv_port table reserves
...
#
# Multi-engine layouts (2p2d/3p1d/1p3d) run several INDEPENDENT decode engines,
...
KV_PORT="${KV_PORT:-36100}"

# Guard against the prefill/decode template mixup: this template is the
# decode config and only valid with tp-size 1.
if [ "$7" != "1" ]; then
    echo "[template] run_dp_template_decode.sh expects tp-size 1 but got $7 -- wrong template for this role (pass the right one via --template)" >&2
    exit 1
fi
...
exec vllm serve /root/model \
    ...
    --compilation-config '{"cudagraph_mode": "FULL_DECODE_ONLY","cudagraph_capture_sizes":[8,32,120]}' \
    --kv-transfer-config \
