#!/bin/sh
# Launches the PD-disaggregation proxy for the A3 3p1d deployment, pre-wired from
# the rank table instead of hand-typed IPs/ports:
#   ranks 0..2 -> prefill: 4 external-DP instances each (kv_producer, 7100..7103)
#                          => 12 prefill endpoints
#   rank  3    -> decode : 16 external-DP instances (kv_consumer, 7100..7115)
#                          => 16 decode endpoints
#
...
setup_rank_env.sh's AISHIPBOX_ADDR_<rank>) for the A3 3p1d layout (ranks 0-2 = 4
prefill instances each on 7100..7103, rank 3 = 16 decode instances on
7100..7115), then execs load_balance_proxy_server_example.py with all 28 endpoints.
