#!/bin/sh
# DeepSeek-V4-Flash P-D disaggregation on A3 (16 NPU/node), layout 1p3d:
# 1 single-node prefill engine + 3 INDEPENDENT single-node decode engines,
...
#   rank 1 -> decode  0  (DP=16 x TP=1, 16 instances 7100..7115, kv_consumer, engine_id=1)
#   rank 2 -> decode  1  (DP=16 x TP=1, 16 instances 7100..7115, kv_consumer, engine_id=3)
#   rank 3 -> decode  2  (DP=16 x TP=1, 16 instances 7100..7115, kv_consumer, engine_id=5)
# Decode-heavy: 1 prefill engine feeds 3 decode engines. kv_port stays 36000
# (prefill) / 36100 (decode); engines are on different nodes so only engine_id
# must be unique (prefill 0 / decode 1,3,5 are disjoint). The proxy fans HTTP
# across all 4 prefill + 48 decode endpoints. Connector: MooncakeHybridConnector;
...
if [ "$AISHIPBOX_NNODES" != 4 ]; then
    echo "[run] expected 4 nodes (1 prefill + 3 decode), rank table has $AISHIPBOX_NNODES" >&2
    exit 1
fi
...
# Print the rank -> role | pod | ip topology so each node logs the full picture.
{
    ...
    i=0
    for role in PREFILL_0 DECODE_0 DECODE_1 DECODE_2; do
...
# engine_id is globally unique: the prefill engine gets an even id (0), decode
# engines get odd ids (1,3,5), derived from this node's index within its role.
case "$AISHIPBOX_NODE_RANK" in
    0) ENGINE_ID=0 exec "$here/run_prefill.sh" ;;
    1) ENGINE_ID=1 exec "$here/run_decode.sh" ;;
    2) ENGINE_ID=3 exec "$here/run_decode.sh" ;;
    3) ENGINE_ID=5 exec "$here/run_decode.sh" ;;
