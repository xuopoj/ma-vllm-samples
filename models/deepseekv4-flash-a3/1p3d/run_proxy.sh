#!/bin/sh
# Launches the PD-disaggregation proxy for the A3 1p3d deployment, pre-wired from
# the rank table instead of hand-typed IPs/ports:
#   rank  0    -> prefill: 4 external-DP instances (kv_producer, 7100..7103)
#                          => 4 prefill endpoints
#   ranks 1..3 -> decode : 16 external-DP instances each (kv_consumer, 7100..7115)
#                          => 48 decode endpoints
#
...
setup_rank_env.sh's AISHIPBOX_ADDR_<rank>) for the A3 1p3d layout (rank 0 = 4
prefill instances on 7100..7103, ranks 1-3 = 16 decode instances each on
7100..7115), then execs load_balance_proxy_server_example.py with all 52 endpoints.
