#!/bin/sh
# Decode node (AISHIPBOX_NODE_RANK=1): external online DP per the official
# A3 1P1D guide — launch_online_dp.py spawns 16 standalone DP=16/TP=1 vllm
# instances (1 NPU each), API ports 7100..7115, this node as DP master
# (rpc port 12321). kv_consumer, kv_port=36100, engine_id=1.
...
echo "[run] role=DECODE_0 (external DP=16 x TP=1, 16 instances) nic=$nic_name local=$local_ip"
