#!/bin/sh
# Decode node 0 (AISHIPBOX_NODE_RANK=1): D-DP master of DP=16, dp-rank 0..7.
# 8 local DP workers (TP=2, 16 NPUs), kv_role=consumer, kv_port=36010. Runs the
# decode API server (the only decode endpoint the proxy talks to). 对齐官方 A3
# PD 参考 run_d0.sh（5.4.2 节）。模型路径改为 /root/model；DP master 地址用本节点
# IP（headless 的 rank 2 拨入此处）。
