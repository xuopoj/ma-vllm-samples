#!/bin/sh
# Launches the PD-disaggregation proxy for the GLM-5.1 A3 1x2P1x4D deployment,
...
#   ranks 2..5 -> decode : 4 external-DP instances each (kv_consumer, 9900..9903)
#                          => 16 decode endpoints (ADDR_2..5:9900..9903)
#
...
setup_rank_env.sh's AISHIPBOX_ADDRS) for the GLM-5.1 A3 1x2P1x4D external-DP
layout (ranks 0-1 = 1 prefill instance each on :9081, ranks 2-5 = 4 decode
instances each on 9900..9903), then execs the official
load_balance_proxy_server_example.py with all 18 endpoints.
...
# split it positionally: ranks 0-1 = prefill nodes, ranks 2-5 = decode nodes.
: "${AISHIPBOX_ADDRS:?run via run.sh}"
# shellcheck disable=SC2086
set -- $AISHIPBOX_ADDRS
if [ "$#" -ne 6 ]; then
    echo "[proxy] expected 6 node addresses in AISHIPBOX_ADDRS, got $# ($AISHIPBOX_ADDRS)" >&2
    exit 1
fi
prefill_addrs="$1 $2"          # ranks 0-1
decode_addrs="$3 $4 $5 $6"     # ranks 2-5
...
# Decode:  4 instances per node on ranks 2-5 (ports DECODE_PORT..DECODE_PORT+3).
...
echo "[proxy] decoders:   16 instances ($decode_addrs):$DECODE_PORT..$((DECODE_PORT + N_DECODE_PER_NODE - 1)) (ranks 2-5)"
