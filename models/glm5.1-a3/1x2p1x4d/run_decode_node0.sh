...
# (DP=16 x TP=4), kv_role=consumer. launch_online_dp.py spawns 4 local instances
# (4 NPUs each, TP=4) on API ports 9900..9903, dp-ranks 0..3, this node as DP
# master (rpc port 10523). The decode engine spans 4 A3 nodes (this master +
...
echo "[run] role=DECODE_0 (external DP=16 x TP=4, 4 local instances) nic=$nic_name local=$local_ip"
