...
# Each invocation is one standalone DP=16/TP=4 instance (4 NPUs). 4 instances per
# decode node; the decode engine spans 4 nodes (DP=16 total).
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeConnectorV1", "kv_role": "kv_consumer", "kv_port": "30100", "kv_connector_extra_config": {"use_ascend_direct": true, "prefill": {"dp_size": 2, "tp_size": 16}, "decode": {"dp_size": 16, "tp_size": 4}}}'
