#!/bin/sh
# Decode per-instance template (official GLM-5 A3 PD guide), called by
...
# decode node; the decode engine spans 2 nodes (DP=8 total) in this 1x2p1x2d
# layout (half the decode of 1x2p1x4d). kv_connector_extra_config's decode
...
exec vllm serve /root/model \
    ...
    --kv-transfer-config '{"kv_connector": "MooncakeConnectorV1", "kv_role": "kv_consumer", "kv_port": "30100", "kv_connector_extra_config": {"use_ascend_direct": true, "prefill": {"dp_size": 2, "tp_size": 16}, "decode": {"dp_size": 8, "tp_size": 4}}}'
