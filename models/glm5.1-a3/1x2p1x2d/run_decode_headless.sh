#!/bin/sh
# Decode worker for node 1 (AISHIPBOX_NODE_RANK=3): DP worker of the decode
# engine (DP=8 x TP=4) led by decode node 0 (rank 2). launch_online_dp.py spawns
# 4 local instances (4 NPUs each, TP=4) on API ports 9900..9903, dp-ranks
...
# Decode nodes are ranks 2..3; the master (rank 2) is run_decode_node0.sh.
...
echo "[run] role=DECODE (external DP=8 x TP=4, 4 local instances, dp-rank-start=$dp_rank_start) nic=$nic_name local=$local_ip master=$decode_master_ip"
