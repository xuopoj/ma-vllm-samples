...
MA 的全局 rank table 机制,用一套统一的脚本完成跨节点、多角色的部署。** 读完你会拿到一套可
...
- 解法:一套与模型无关的 **基础脚手架(spine)**(`setup_rank_env.sh` + `check_hccn.sh`)+ 一个**分发器** `run.sh`,按 rank 决定角色。
- 复杂度沿 `1node → 2nodes → pd-1p1d` 递进,但基础脚手架始终不变。
...
## 3. 基础脚手架第一块:`setup_rank_env.sh` —— 监听并解析 rank table
...
## 4. 基础脚手架第二块:`check_hccn.sh` —— 启动前的网络体检（可选）
...
每个角色脚本结构相同:**相同的 NIC/socket 前导(基础脚手架的一部分)+ 各自的 `vllm serve` 参数块**。
...
## 8. 基础脚手架是 source of truth:`template/`

与模型无关的构建块——`setup_rank_env.sh` 和 `check_hccn.sh`——以
...
```bash
md5 template/setup_rank_env.sh */*/*/*/setup_rank_env.sh   # 必须全部一致
```
...
**只有 `deepseekv4/flash/a3` 的 `1node` / `2nodes` / `pd-1p1d` 这几个 layout 经过真机端到端
验证。** 其余的——包括 `flash/a3/pd`——都是**派生但未测试**的:是起点,而非已知可用的配置。
在依赖任何 `⚠` layout 之前,先在**每个节点**跑 `check_hccn.sh`、确认 API 能正常响应,做完后
更新本矩阵。
...
经验证的 `deepseekv4/flash/a3` layout 是**恢复基线**:当某个未测试的部署出问题时,把它的
`run_*.sh` 与对应的已验证 layout 做 diff,定位它从哪里开始不一致。
