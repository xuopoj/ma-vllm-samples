...
(`deepseekv4-flash-a3`、`glm5.1-a3`、`qwen3-32b-a3`);未标变体的模型省略该段。
...
**只有 `deepseekv4-flash-a3` 的 `1node` / `2nodes` / `1p1d` 这几个 layout 经过真机端到端
验证。** 其余的都是**派生但未测试**的:是起点,而非已知可用的配置。在依赖任何 `⚠` layout
之前,先在**每个节点**确认引擎正常起来、API 能正常响应,做完后更新本矩阵。

| 目录 | Status |
|------|--------|
| `models/deepseekv4-flash-a3/1node` | ✅ verified |
| `models/deepseekv4-flash-a3/2nodes` | ✅ verified |
| `models/deepseekv4-flash-a3/1p1d` | ✅ verified |
| `models/deepseekv4-flash-a3/2p1x2d` | ⚠ untested |
