"""官方编排器(原样复制):按 dp-size-local fork N 个进程,每个进程用 bash 调
   单实例模板(run_dp_template_*.sh),分配各自的 NPU / 端口 / dp-rank。
   来源:vLLM-Ascend DeepSeek-V4-Flash 1P1D 教程。"""
import argparse
...
def parse_args():
    ...
    parser.add_argument("--template", type=str, default="./run_dp_template.sh", help="Per-instance launch template.")
...
def run_command(visible_devices, dp_rank, vllm_engine_port):
    command = [
        "bash", template_path,
        visible_devices, str(vllm_engine_port), str(dp_size),
        str(dp_rank), dp_address, dp_rpc_port, str(tp_size),
    ]
...
if __name__ == "__main__":
    ...
    for i in range(dp_size_local):
        ...
        # 第 i 个实例独占 [i*tp_size, (i+1)*tp_size) 这几张卡
        visible_devices = ",".join(str(x) for x in range(i * tp_size, (i + 1) * tp_size))
        p = multiprocessing.Process(target=run_command, args=(visible_devices, dp_rank, vllm_engine_port))
        processes.append(p)
        p.start()
