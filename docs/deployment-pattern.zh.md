# 在 Atlas 800 上用 vLLM-Ascend 部署大模型:rank table 监听与分发模式

> 从单机到 PD 分离,如何用一套自动化节点发现脚本统一多机多角色的启动流程。

如今,**部署第三方开源大模型**已经成为越来越普遍的落地场景。GLM、DeepSeek 这类参数量巨大的 MoE 模型不断涌现,它们大多**单机放不下**——要么需要跨节点的张量/数据并行,要么需要 prefill/decode 分离这样的多角色部署。而这恰恰是工程上最容易出错的环节:同一份脚本要在多台机器上扮演不同角色,节点地址在运行时才分配,流量路由还有平台层面的约束。

本文面向**在华为云 ModelArts(MA)上做大模型推理部署的工程师**,要讲清楚一件事:**如何结合 MA 的全局 rank table 机制,用一套统一的脚本完成跨节点、多角色的部署。** 读完你会拿到一套可直接复用的模式——从监听 rank table、按角色分发(`run.sh`),到网络体检与引擎启动,并能把它从单机平滑扩展到 PD 分离的多机集群。
...
- 解法:一套与模型无关的 **spine**(`setup_rank_env.sh` + `check_hccn.sh`)+ 一个**分发器** `run.sh`,按 rank 决定角色。
- 复杂度沿 `1node → 2nodes → pd-1p1d` 递进,但 spine 始终不变。
...
flowchart TD
    ...
    subgraph Spine["setup_rank_env.sh(spine)"]
...
> 上图以 `2nodes`(双机混合引擎)为例。`pd-1p1d` 只是分发分支变多(prefill / decode / proxy),监听与解析这一段完全一样。
...
- **多角色脚本易漂移。** prefill / decode 等角色若各写一份脚本,公共部分(网络初始化、rank 解析)极易不一致、难维护。
...
## 3. spine 第一块:`setup_rank_env.sh` —— 监听并解析 rank table
...
> **关键约定:必须 `source`,不能 `&&` 链式调用。**
> `sh setup_rank_env.sh && vllm serve ...` 不生效——导出的变量随子 shell 一起消失。
> 要么 `. setup_rank_env.sh`(source),要么用 exec 形式 `sh setup_rank_env.sh vllm serve ...`。

---

## 4. spine 第二块:`check_hccn.sh` —— 启动前的网络体检（可选）

多机 HCCL 通信依赖 NPU 间网络(HCCN)。启动引擎前,可在**每个节点跑一遍**并互相比对输出:

```bash
sh /root/script/check_hccn.sh                 # 本机检查
sh /root/script/check_hccn.sh <peer_npu_ip>   # 加跨节点 ping
sh /root/script/check_hccn.sh --rank 1        # 从 $AISHIPBOX_ADDR_1 解析 ping 目标
```

脚本会自动识别平台(A2=8 卡 / A3=16 卡,按 `/dev/davinci*` 数量),做 6 步检查:链路健康、HCCN 配置、NPU IP、superpod(A3)、跨节点 ping、TLS 设置。

**这一步是可选的,而且结果只能作参考,原因有三:**

- **部分命令容器内跑不了。** `hccn_tool` / `npu-smi` 来自宿主机 NPU 驱动,容器没挂载驱动目录就无法执行（一般会挂载）;有些查询还需要特权。脚本对此做了容错(找不到工具会提示挂载方式),但能跑到哪一步取决于容器权限。
- **检查项随网络结构而变。** 是否有 RoCE、是否超节点(superpod)、A2 还是 A3,对应的检查命令和预期结果都不同——没有一套"放之四海皆准"的判定。脚本只是把官方文档里各场景的检查尽量自动化,不代表覆盖了你的具体组网。
- **结果需要专业解读。** 输出里什么算正常、什么是告警,往往要**升腾(Ascend)侧的同事**结合实际组网来判断。本脚本的定位是"快速采集 + 横向比对各节点",而非给出最终结论。

所以把它当作**启动前的快速体检和信息采集**:跑一遍、各节点比对输出(链路是否 `UP`、TLS `switch` 是否一致),有疑点再拉升腾的人一起看。它不是部署的硬性前置步骤。

---

## 5. 分发器:`run.sh` —— 按 rank 决定角色

这是把上面所有东西串起来的入口。逻辑三步走:

```sh
here=$(cd "$(dirname "$0")" && pwd)
. "$here/setup_rank_env.sh"        # 1. source spine,等 rank table + 导出 AISHIPBOX_*
...
## 6. 角色启动脚本:`run_node*.sh`

每个角色脚本结构相同:**相同的 NIC/socket 前导(spine 的一部分)+ 各自的 `vllm serve` 参数块**。
...
## 7. 复杂度递进:同一套 spine,三种 layout

整个模式最有价值的地方:**spine 不变,只是 `run_*.sh` 的数量和角色在变。**

| Layout | 拓扑 | 并行（示例） | 脚本构成 |
|--------|------|------|----------|
| **1node** | 单机 | DP=4×TP=4 | 仅 `run.sh`(无 rank table) |
| **2nodes** | 双机混合引擎 | DP=8×TP=4 | `run.sh` + `run_node{0,1}.sh` |
| **pd-1p1d** | 1P1D 分离 + 外置 DP | P:4/4×4, D:16/1×16 | + `launch_online_dp.py` + `run_dp_template_<role>.sh` + `run_proxy.sh` |


### 7.1 从 2nodes 到 pd-1p1d:PD 分离 + 外置 online DP

`2nodes` 是一个**混合引擎**:同一批卡既做 prefill 又做 decode。`pd-1p1d` 把两者**拆成独立角色**——prefill 专注吃 prompt、产出 KV cache,decode 专注消费 KV、出 token,两者各自调优、各自扩缩容;并且更进一步,**每个 DP rank 都是一个独立的 vllm 进程、各自一个 API server**,再由 proxy 在所有实例间负载均衡。这种"外置 online DP"在隔离性和扩缩容上更灵活。

以本仓库的 `pd-1p1d` 为例,2 个 rank:
...
> 还有一个分发上的小差异:`pd-1p1d` 的 `run.sh` 允许 `GROUP0_SIZE` 为 1 或 2,并且**凡是 group 0 的节点都会顺带在后台拉起 proxy**(`sh run_proxy.sh &`)——proxy 不再占一个独立 rank,而是和引擎同节点共存。
...
### 5. `check_hccn.sh` 一定要先跑吗?

不是硬性前置,**可选**。它用于启动前快速采集网络信息、横向比对各节点,但部分命令容器内跑不了、检查项随组网而变、结果还需升腾侧专业解读(见 §4)。建议跑一遍留底,有疑点再拉升腾的人一起看。
