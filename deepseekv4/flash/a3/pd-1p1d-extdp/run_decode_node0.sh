#!/bin/sh
# Decode node (AISHIPBOX_NODE_RANK=1): external online DP — 16 standalone
...
# All instances share kv_port=36100 (occupies 36100..36115), engine_id=1.
...
echo "[run] role=DECODE_0 (external DP=16, 16 instances) nic=$nic_name local=$local_ip"
...
while [ "$i" -lt "$DP_SIZE" ]; do
    ...
    echo "[run] launching decode dp-rank $i on NPU $i, API port $port"
    sh "$here/run_dp_decode_template.sh" "$i" "$port" "$DP_SIZE" "$i" "$local_ip" "$DP_RPC_PORT" "$TP_SIZE" &
    pids="$pids $!"
    i=$((i + 1))
done
...
while :; do
    for p in $pids; do
        if ! kill -0 "$p" 2>/dev/null; then
            echo "[run] decode DP instance (pid $p) exited -- shutting down the rest" >&2
