#!/bin/sh
# Prefill leader (AISHIPBOX_NODE_RANK=0): DP=16 engine spanning nodes 0-1,
# kv_role=producer. This node is the DP leader and runs dp-ranks 0..7 (one per
# local NPU, TP=1); node 1 joins headless with dp-ranks 8..15. EP spans all 16
# NPUs, so each NPU holds half the experts vs the standalone DP=8 layout.
# kv_port=30000, engine_id=0, API server on :7100 (this node only).
...
echo "[run] role=PREFILL_LEADER nic=$nic_name local=$local_ip"
